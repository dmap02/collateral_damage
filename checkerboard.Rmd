...
let's try the mediation analysis

Mediation framework here:

    Predictor (X): Van (vancomycin concentration)

    Mediator (M): pH (qualitative but treated numerically or factor)

    Outcome (Y): Growth
    
    ACME (Average Causal Mediation Effect): Effect of Van on Growth through pH (the mediated path)

    ADE (Average Direct Effect): Effect of Van on Growth not through pH (the direct path)

    Total Effect: Sum of direct + mediated effects

    Proportion Mediated: How much of the total effect is due to mediation by pH
    
  
```{r}
# Mediator model (predict pH by Van)
med.mod <- lm(pH ~ VanConc +FluConc, data = data4)

# Outcome model (predict Growth by Van and pH)
out.mod <- lm(GrowthPercent ~ VanConc + pH + FluConc, data = data4)

#run the mediation
med.out <- mediate(med.mod, out.mod, treat = "VanConc", mediator = "pH", boot = TRUE, sims = 1000)
summary(med.out)
plot(med.out)
sensitivity <- medsens(med.out)
plot(sensitivity)
...
summary(mod_growth)
```
let's plot some regression diagnostics
```{r}
library(ggfortify)
autoplot(mod_growth)
```
