...
get the average increase in growth
```{r}
# Convert to numeric matrix if it's not already
avg_mat_numeric <- as.matrix(average_matrix)

# Subtract 100 from each value to get the increase over baseline
increase_matrix <- avg_mat_numeric - 100

# Remove the baseline row (since it’s all 0s after subtraction)
increase_matrix_no0 <- increase_matrix[-1, ]

# Compute the average increase across columns for each Vancomycin concentration
average_increase_by_vanco <- rowMeans(increase_matrix_no0)
average_increase_by_vanco
```
...
While holding pH and Fluconazole concentration constant in the model, the observed effect of Vancomycin concentration on growth percentage was substantially greater than expected under the null hypothesis. In a permutation test with 1000 random shuffles of Vancomycin concentration, only 4 permutations produced p-values smaller than the observed p-value, indicating that the likelihood of observing such a strong effect by chance is very low (empirical p-value = 0.004), much lower than the typical 5% threshold used to delcare significance. Additionally, the effect sizes were all significantly less than the observed effect size. This provides strong evidence that Vancomycin concentration has a significant influence on growth, beyond its effects on pH, and is unlikely to be a false positive.
...
### try to permute vancomycin concentrations after removing 25, 50

even when we remove these big outliers the effects are still significant

```{r}
set.seed(123)
n_perms <- 1000
data5 = subset(data4, !(VanConc %in% c(25, 50)))
...
for (i in 1:n_perms) {
  data5$VanConc_perm <- sample(data5$VanConc)  # shuffle VanConc

  # Fit permuted model
  perm_mod <- lm(GrowthPercent ~ VanConc_perm + pH + FluConc, data = data5)
...
# Fit original model (unpermuted)
obs_mod <- lm(GrowthPercent ~ VanConc + pH + FluConc, data = data5)
obs_coef <- coef(obs_mod)["VanConc"]
obs_p <- summary(obs_mod)$coefficients["VanConc", "Pr(>|t|)"]
autoplot(obs_mod)
# Plot histogram of permuted VanConc coefficients with observed value line
